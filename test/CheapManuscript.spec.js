...
var CheapManuscript = artifacts.require('../contracts/CheapManuscript.sol')
var ManuscriptFactory = artifacts.require('../contracts/ManuscriptFactory.sol')

contract('CheapManuscript', function (accounts) {
  ...
  it('creates manuscripts', async function() {
    ...
    instanceMaFactory = await ManuscriptFactory.deployed();
    ...
    for(var cnt=4; cnt<6; cnt++) {
      manuscript[cnt] = await CheapManuscript.new(bytesOfAddress[cnt]);
      addressManuscript[cnt] = await manuscript[cnt].address;
    }

    for(var cnt=6; cnt<7; cnt++) {
      await instanceMaFactory.createManuscript(bytesOfAddress[cnt]);
      addressManuscript[cnt] = await instanceMaFactory.manuscriptRegistry(bytesOfAddress[cnt]);
      manuscript[cnt] = await MinimalManuscript2.at(addressManuscript[cnt]);
      console.log(addressManuscript[cnt])
      console.log(await manuscript[cnt].owner());
      console.log(accounts[0])
      manuscript[cnt].addAuthor(accounts[1])
    }
