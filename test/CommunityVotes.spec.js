...
describe('ComunityVotes', function() {
  ...
  it('vote for and against manuscript', async function() {
    ...
    for(var cnt=0; cnt<9; cnt++) {
      ...
      if (await instance.votingActive(bytesOfAddress[0]) > 0 ) {
        // vote
        console.log(bytesOfAddress[0], accounts[cnt], vote)
    ...
    const blah = await instance.votingActive(bytesOfAddress[0])
